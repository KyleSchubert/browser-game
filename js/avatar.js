...
function testing() {
    ...
    makeTestPixel(neck[0], neck[1], 'blue');
    makeTestPixel(neck[0]-4, neck[1]-19, 'blue');
    makeTestPixel(neck[0]-4, neck[1]+11, 'blue');
    cyclicAvatarAnimate(body, bodyState);
    cyclicAvatarAnimate(head, headState);
    cyclicAvatarAnimate(face, faceState);
}

function makeTestPixel(x, y, color='red') {
    ...
    Object.keys(bodyData[id]).forEach((state) => {
        bodyData[id][state].forEach((frame) => {
            frame.forEach((part) => {
                ...
                makeTestPixel(offsets[0], offsets[1]);
